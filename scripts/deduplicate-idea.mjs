...
const CONCEPT_TOKEN_THRESHOLD = 0.34;
const CONCEPT_BIGRAM_THRESHOLD = 0.06;
const GLOBAL_CONCEPT_TOKEN_THRESHOLD = 0.52;
...
const ideaConceptText = compactConceptText(idea);
...
for (const entry of entries) {
  ...
  const historyConceptText = compactConceptText(entry);
  const concept = conceptSimilarity(ideaConceptText, historyConceptText);
  if (
    concept.token >= GLOBAL_CONCEPT_TOKEN_THRESHOLD
    || (
      ideaSector
      && ideaSector === String(entry.sector || '')
      && concept.token >= CONCEPT_TOKEN_THRESHOLD
      && concept.bigram >= CONCEPT_BIGRAM_THRESHOLD
    )
  ) {
    match = {
      runFolder,
      reason: `concept-similarity-token-${concept.token.toFixed(2)}-bigram-${concept.bigram.toFixed(2)}`,
    };
    break;
  }
}
