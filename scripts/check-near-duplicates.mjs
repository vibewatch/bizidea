...
function candidateReasons(a, b) {
  ...
  const concept = conceptSimilarity(
    compactConceptText(a),
    compactConceptText(b),
  );
  ...
  if (
    concept.token >= 0.52
    || (sameSector && concept.token >= 0.34 && concept.bigram >= 0.06)
  ) {
    reasons.push(`concept similarity token ${concept.token.toFixed(2)}, bigram ${concept.bigram.toFixed(2)}`);
  }
  ...
  return {
    reasons,
    score: sharedEventKeys.length * 100
      + sharedCompanies.length * 20
      + pitchSimilarity * 10
      + slugSimilarity * 10
      + concept.token * 20
      + concept.bigram * 20
...
function main() {
  ...
  const pitchFrames = new Map();

  for (const entry of entries) {
    if (Number(entry.qualityPolicyVersion || 1) < 2) continue;
    const cliche = findTitleCliche(entry.slug, entry.pitch);
    if (cliche || /(?:^|-)os(?:-|$)/i.test(String(entry.slug || ''))) {
      policyFailures.push(`${entry.runFolder}: title cliché`);
    }
    const frame = String(entry.pitchFrame || '').trim();
    if (frame) {
      const previous = pitchFrames.get(frame);
      if (previous) policyFailures.push(`${entry.runFolder}: repeats pitch opener "${frame}" from ${previous}`);
      else pitchFrames.set(frame, entry.runFolder);
    }
  }
  ...
  for (let i = 0; i < entries.length; i += 1) {
    for (let j = i + 1; j < entries.length; j += 1) {
      ...
      if (Number(a.qualityPolicyVersion || 1) >= 2 || Number(b.qualityPolicyVersion || 1) >= 2) {
        const blockingReasons = reasons.filter((reason) => (
          reason.startsWith('pitch similarity')
          || reason.startsWith('concept similarity')
          || reason.startsWith('same company/sector')
        ));
        if (blockingReasons.length > 0) {
          policyFailures.push(`${a.runFolder} ↔ ${b.runFolder}: ${blockingReasons.join('; ')}`);
        }
      }
    }
  }
  ...
  if (candidates.length === 0 && policyFailures.length === 0) {
    console.log(`[near-duplicates] ✓ no review candidates across ${entries.length} entries.`);
    return;
  }

  console.log(`[near-duplicates] ${candidates.length} historical review candidate(s) across ${entries.length} entries:`);
  ...
  if (policyFailures.length > 0) {
    console.error(`[near-duplicates] ✗ ${policyFailures.length} current-policy violation(s):`);
    for (const failure of policyFailures.slice(0, 30)) console.error(`  - ${failure}`);
    if (policyFailures.length > 30) console.error(`  - ...and ${policyFailures.length - 30} more`);
    process.exit(1);
  }
