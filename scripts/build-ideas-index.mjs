...
import { STOPWORDS, words } from './text.mjs';
...
function build() {
  ...
  for (const runFolder of runs) {
    ...
    const wedge = String(idea?.startupThesis?.wedge || '');
    ...
    entries.push({
      ...
      wedge,
      targetUserPrimary,
      pitchFrame: words(pitch).slice(0, 3).join(' '),
      qualityPolicyVersion: Number(idea?.qualityPolicyVersion || 1),
      selectionLens: idea?.selectionLens || null,
