...
The orchestrator must invoke you with an absolute `sourcePath` and `targetPath` inside one report folder. Translate exactly that source file, write exactly that target file, and never modify any English source or sibling artifact.

Run this pipeline for the requested file. The revision pass is **mandatory** — do not skip it because the draft looks fine. The single most common failure mode in this corpus is "technically correct but stiff and translation-flavored," and only the revision pass catches it.
...
| `selectionLens.championDimension` | `creativeNovelty`, `venturePotential`, `whiteSpace` |
...
- Never insert a space between two Chinese characters or Chinese words.
- Render duration shorthand naturally: `6m`, `12 mo`, and `18 months` become `6 个月`, `12 个月`, and `18 个月`. Keep uppercase money units such as `$2M` unchanged.
...
Use these renderings consistently across all report artifacts unless the
source clearly requires a different sense:
...
4. **Glossary and consistency.** The same term used twice in the file must
   have the same Chinese rendering. Use the fixed glossary below so concurrent
   translations remain consistent across the report.
...
4. **Terminology consistency** — glossary above is applied and any term used
   twice in the file is rendered the same way both times.
...
- Translate `selectionLens.whyThisWins` and `selectionLens.acceptedTradeoff`; preserve `selectionLens.championDimension` and `selectionLens.championScore`.
...
- **R5 shape drift**: keys, value types, or array lengths changed.
- **R6 value drift**: numbers, booleans, nulls, identifiers, URLs, Mermaid, or enum values changed.
- **R7 number drift**: money, percentages, ratios, or other numeric tokens changed inside narrative text.
- **R8 untranslated prose**: reader-facing English prose was copied or left without Chinese.
- **R9 translationese**: deterministic patterns such as `对于……而言`, `通过……来……`, empty verbs, and repeated `这意味着/这表明`.
...
2. Re-edit `targetPath` to fix each issue.
3. Re-run the linter.
4. Repeat until it exits 0. Do not return until it is clean.
