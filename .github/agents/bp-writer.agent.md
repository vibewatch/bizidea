...
6. Run `node scripts/validate-stage.mjs <folder> business-plan` from the repo root and confirm it exits zero. If it fails, fix the file and re-run before returning.
...
GitHub Copilot returns your final response to the parent agent natively. After the validator passes, state the absolute path, a two-sentence executive-summary excerpt, the strategic tradeoff preserved from `selectionLens.acceptedTradeoff`, and the validation result. On failure, state the reason plainly and remove any invalid partial file. Do not emit a custom protocol block.
