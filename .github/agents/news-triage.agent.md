...
8. **Dedupe** each cluster against the per-entry history records from step 2. The history index records `eventKeys` with an event-type derived from a text heuristic (see [`scripts/build-ideas-index.mjs`](../../scripts/build-ideas-index.mjs)) that may not match this run's classification, so use event type as corroborating evidence but do not let `<companyLowercased>|<YYYY-MM>` alone block a genuinely different same-company event. Continue to record the full `<companyLowercased>|<eventType>|<YYYY-MM>` form in the cluster's `eventKeys` field for the audit trail.
