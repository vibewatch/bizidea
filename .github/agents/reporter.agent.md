...
3. Copy `idea.yaml.selectionLens` exactly into `index.yaml.selectionLens`; this is the primary explanation for why the concept was selected.
4. From `idea.yaml.sourceContext`: the `topic` prompt (verbatim), `timeWindow`, and `timeWindowLabel`.
5. From `research.yaml`: `market.tam.value`, `market.sam.value`, `market.som.value` (preserve units like `$0.37B–$0.66B`, `$10M`).
6. From `financial-model.yaml`: `totals.y1`/`y2`/`y3` for revenue, EBITDA, ending cash; `fundingAsk.amountM`, `fundingAsk.round`, `fundingAsk.runwayMonths`.
7. Derive `kicker` from the topic. The website renders it as a short strap label (e.g. on cards joined to the sector with `·`), so keep it tight:
...
8. **Apply the rating rubric** (next section) using the four stage YAML files you just read.
9. Write `index.yaml` matching the schema exactly. Use YAML with 2-space indent. Any field present in an upstream stage YAML but not listed in the schema below MUST be ignored.
10. Run `node scripts/validate-stage.mjs <folder> index` from the repo root and confirm it exits zero. If it fails, fix the file and re-run before returning.
...
Compute `overall` = the weighted sum `0.30 * market + 0.30 * differentiation + 0.25 * execution + 0.15 * timeliness`, then round half-up to one decimal place (e.g. `3.55` → `3.6`, `3.54` → `3.5`). This secondary report rating does not decide selection; `selectionLens` records the champion reason. Always include all four dimensions even if you must default to `3` with rationale `"insufficient data"`.
...
selectionLens:
  ...
  championScore: 5
  whyThisWins: copied exactly from idea.yaml
  acceptedTradeoff: copied exactly from idea.yaml
files:
  idea: idea.yaml
...
GitHub Copilot returns your final response to the parent agent natively. After the validator passes, state the absolute path, slug, sector, overall rating, champion dimension, null fields, and validation result. On failure, state the reason plainly and remove any invalid partial file. Do not emit a custom protocol block.
